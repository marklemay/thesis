...
The full rule for function reduction is given in \rulename{\Rrightarrow-\mathsf{fun}-::-red} which makes the behavour from the examples explicit: agrument types are swapped as a term is applied under a cast.
...
\begin{casenv}
  ...
  \begin{casenv}
    ...
    \begin{tabular}{llll}
      ...
      % \makecell[l]{$H\vdash$ \\ $\left(b'\ ac\right)::_{B_{1}'\left[x\coloneqq ac\right],\ell ,o'.bod[a']}B_{2}'\left[x\coloneqq a'\right]$} & by \rulename{cast-::} \tabularnewline
      \end{tabular}
      Which allows us to conclude $H\vdash\left(b'\ ac\right)::_{B_{1}'\left[x\coloneqq ac\right],\ell ,o'.bod[a']}B_{2}'\left[x\coloneqq a'\right]$ by \rulename{cast-::}.

    \item other reductions allong the lines of \ch{2}
  \end{casenv}
  \item All other cases follow allong the lines of \ch{2}
\end{casenv}
...
The limited amount of observation makes the system in this chapter substantually simpler then the system in \ch{5} where more observations are possible.
...
\begin{casenv}
  ...
  \item \rulename{ty-conv} follows by induction and that $\equiv$ is an equivelence 
  ...
  \item \rulename{cast-::} is impossible.
    ...
    Wchich contridicts the sidecondition of \rulename{cast-::}.
...
\begin{casenv}
  ...
  \item \rulename{cast-::} then it must be a value from \rulename{Val-::} setisfieng the 2nd conclusion
  \item \rulename{ty-conv} follows by induction and teh transitivity of $\equiv$
...
For instance\footnote{assuming the data types of \ch{5}}, $\mathtt{head}\,\mathbb{B}\,1\,\left(\mathtt{rep}\,\mathbb{B}\,true\,0\right)$ will result in blame since $1$ and $0$ have different head constructors.
