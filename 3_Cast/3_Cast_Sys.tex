...
The only rule that types differently than the \slang{} is the \rulename{cast-::} rule that allows runtime type assertions.
...
The explicit new rules for $\textbf{max}$ are given in \Fref{cast-sys-max} with the structural rules omitted\footnote{
  They are the same as \ch{2}.
}.
