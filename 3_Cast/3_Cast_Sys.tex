...
The explicit new rules for $\textbf{max}$  are given in \Fref{cast-sys-max} with the structural rules from \ch{2} omitted.
...
\end{tabular}
\todo{actually have all the rules but gray out the redundant ones}
\caption{$\textbf{max}$}
...
The limited amount of observation makes the system in this chapter substantially simpler than the system in \ch{5} where more observations are possible.
