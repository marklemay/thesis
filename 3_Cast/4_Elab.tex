...
Note that the location tags match almost exactly\todo{or will after rewritten} with the check annotations of the bidirectional system.
...
All the meta theory from Chapter 2 goes through assuming that all locations are equivalent and by generating null locations when needed\footnote{
 ...
 $\frac{M\Rrightarrow M'\quad N\Rrightarrow N'}{\left(x:M_{l}\right)\rightarrow N_{l'}\Rrightarrow\left(x:M'_{l''}\right)\rightarrow N'_{l'''}}\,\textrm{\Rrightarrow -fun-ty}$,
 ...
 so that the output is unique.}\todo{fn a little awk}.
...
As with bidirectional type checking variable types will be inferred by casting the outer type.
For example,
\begin{example}
