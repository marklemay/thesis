...
\geometry{verbose,tmargin=1.5in,bmargin=1in}
...
\begin{document}
\title{Chapter 3 (draft): the Dependent Cast System}
\author{Mark Lemay}
\maketitle
...
\section{Alternative Elab formalization}
...
The $\operatorname{\overleftarrow{\textbf{Elab}}-conv-\star}$ rule
is a bit of a hack, and does not reflect the implementation. This
formalization allows us to remove the special case of the conversion
rule.The above is more accurate, but harder to prove. and proofs are
very sensitive to the exact formalization of bidirectional type checking.
seems more complicated to prove, though such a proof might be more
enlightening.

It may be possible to prove one of the elaboration properties by stages,
for instance with a lemma like $\Gamma\vdash m:M$ (some constraint
on gamma) $H\vdash m\,\textbf{Elab}\ a\overrightarrow{\,:\,}A$ where
$a$cannot cause blame. Then use that as a lemma in the main thing

% uhoh
The elaboration relation is mostly well behaved, but as presented
here, it is undecidable for some pathological terms. Because the application
rule follows the bidirectional style, we may need to determine if
a type level computation results in a function type. If that computation
``runs forever'', elaboration will ``run forever''. If we did
not allow general recursion (and the non-termination allowable by
type-in-type), we suspect elaboration would always terminate.
...
\section{Alt construction}

consider making the cast system intrinsically typed? or intrinsically
typed enough that casts can easily be removed

\section{Other}
\begin{itemize}
\item properly complete the ``correctness guarantees''
\begin{itemize}
\item the warning scheme hints at the induction invariant that is needed,
not only are the casts types generated by well typed code similar,
every cast inside of those labels are similar
\item this should hold for terms the TAS and check, which is prob easier
to work with.
\end{itemize}
\item TODO relate to https://hal.archives-ouvertes.fr/hal-01849166v3/document
? 
\item locations are ``correct by construction''
\item In every popular type system users are allowed to assume unsafe equalities.
Thus ...
\end{itemize}
...
\label{fig:cast-ctx-wf}
\end{figure}

We will often want to limit ourselves to well formed contexts, which
are defined in \ref{fig:cast-ctx-wf} . This definition excludes nonsensical
contexts like $x:3_{c}\vdash$. Sensible contexts will be required
for some further proofs.

Because the derivations were chosen to simplify the type soundness
proof, the $\equiv$ relation is untyped and thus untyped terms can
be associated with typed terms. For instance, $\star\equiv\left(\lambda-\Rightarrow\star\right)\left(\star\,\star\right)$,
(you can replace $\left(\star\,\star\right)$ with any untypeable
expression). Which leads to junk typing judgments like $\vdash\star:\left(\lambda-\Rightarrow\star\right)\left(\star\,\star\right)$,
while not $\vdash\left(\lambda-\Rightarrow\star\right)\left(\star\,\star\right):\star$
. Because of this we will sometimes need to assume $H\vdash A:\star$,
in addition $H\vdash a:A$.

\section{...}

However in a realistic implementation the function application would
reduce the function type to weak head normal form (to match a realistic
bidirectional implementations) and will be undecidable for some pathological
terms. Because the application rule needs to determine if a type level
computation results in a function type. If that computation ``runs
forever'', elaboration will ``run forever''. 

\section{...}

TODO can $\Gamma\textbf{Elab}H$ be made simpler?

by mutual induction on the bi-directional typing derivations and the
well formedness of $\Gamma\vdash$

\begin{tabular}{lll}
$\operatorname{\overrightarrow{ty}-var}$ & $x:M\in\Gamma$ & \tabularnewline
 & ... & mutual induction\tabularnewline
 & $x:A\in H$ & \tabularnewline
 & $H\vdash x\,\textbf{Elab}\,x\overrightarrow{\,:\,}A$ & $\operatorname{\overrightarrow{\textbf{Elab}}-var}$\tabularnewline
$\operatorname{\overrightarrow{ty}-\star}$ &  & \tabularnewline
 & ... & mutual induction\tabularnewline
 & $H\vdash\star\,\textbf{Elab}\,\star\overrightarrow{\,:\,}\star$ & $\operatorname{\overrightarrow{\textbf{Elab}}-\star}$\tabularnewline
$\operatorname{\overrightarrow{ty}-::}$ & $\Gamma\vdash m\overleftarrow{\,:\,}M$, $\Gamma\vdash M\overleftarrow{\,:\,}\star$,
for some $\ell$ & ( $\ell$ comes from the syntactic extension to the surface lang)\tabularnewline
 & $H\vdash m\overleftarrow{\,:_{\ell,.}\,}A\,\textbf{Elab}\,a$ & by induction, $o=.$\tabularnewline
 & $\Gamma\vdash M\overleftarrow{\,:\,}\star$ & by regularity\tabularnewline
 & $H\vdash M\overleftarrow{\,:_{\ell.}\,}\star\,\textbf{Elab}\ A'$ & by induction, $o=.$\tabularnewline
 & m\textasciitilde a, M\textasciitilde A, m:M then a:A & TODO\tabularnewline
 & $H\vdash\left(m::_{\ell}M\right)\textbf{Elab}\,a\overrightarrow{\,:\,}A$ & $\operatorname{\overrightarrow{\textbf{Elab}}-::}$\tabularnewline
$\operatorname{\overrightarrow{ty}-fun-ty}$ & \multicolumn{2}{l}{$\Gamma\vdash M\overleftarrow{\,:\,}\star\quad\Gamma,x:M\vdash N\overleftarrow{\,:\,}\star$,
with $\ell$, $\ell'$}\tabularnewline
 & $H\vdash M\overleftarrow{\,:_{\ell,.}\,}\star\textbf{Elab}\ A$ & by induction, $o=.$\tabularnewline
 & $H,x:A\vdash N\overleftarrow{\,:_{\ell',.}\,}\star\textbf{Elab}\ B$ & ...M\textasciitilde A\tabularnewline
 & $H\vdash\left(\left(x:M_{\ensuremath{\ell}}\right)\rightarrow N_{\ensuremath{\ell'}}\right)\textbf{Elab}\left(\left(x:A\right)\rightarrow B\right)\overrightarrow{\,:\,}\star$ & $\operatorname{\overrightarrow{\textbf{Elab}}-fun-ty}$\tabularnewline
$\operatorname{\overrightarrow{ty}-fun-app}$ & \multicolumn{2}{l}{$\Gamma\vdash m\overrightarrow{\,:\,}\left(x:N\right)\rightarrow M\quad\Gamma\vdash n\overleftarrow{\,:\,}N$,
with $\ell$}\tabularnewline
 & $H\vdash m\,\textbf{Elab}\ b\overrightarrow{\,:\,}C$ & by induction\tabularnewline
 & $C\Rrightarrow_{\ast}\left(x:A\right)$ & ???\tabularnewline
 & $H\vdash n\overleftarrow{\,:_{\ell,.arg}\,}A\,\textbf{Elab}\,a$ & by induction\tabularnewline
 & $H\vdash\left(m_{\ell}\,n\right)\textbf{Elab}\left(b\,a\right)\overrightarrow{\,:\,}B\left[x\coloneqq a\right]$ & $\operatorname{\overrightarrow{\textbf{Elab}}-fun-app}$\tabularnewline
 &  & \tabularnewline
TODO: Mutual cases &  & \tabularnewline
\end{tabular}

...

The cast language records all the potential type conflicts, and uses
runtime annotations to monitor these discrepancies. If the types do
not conflict at runtime, evaluation will continue, otherwise the system
will correctly blame the source location of the conflict with a concrete
inequality. Terms that type according to the cast language type system
will be referred to as \textbf{well casts}.
...
$true_{c}\doteq_{\mathbb{B}_{c}}false_{c}\coloneqq\left(C:\left(\mathbb{B}_{c}\rightarrow\star\right)\right)\rightarrow C\,true_{c}\rightarrow C\,false_{c}$ & $\left(refl_{true_{c}:\mathbb{B}_{c}}::_{true_{c}\doteq_{\mathbb{B}_{c}}true_{c}}\left(C:\left(\mathbb{B}_{c}\rightarrow\star\right)\right)\rightarrow C\,true_{c}\rightarrow C\,false_{c}\right)\,toLogic\,tt_{c}$\tabularnewline
$true_{c}\doteq_{\mathbb{B}_{c}}true_{c}\coloneqq\left(C:\left(\mathbb{B}_{c}\rightarrow\star\right)\right)\rightarrow C\,true_{c}\rightarrow C\,true_{c}$ & $\left(refl_{true_{c}:\mathbb{B}_{c}}::_{\left(C:\left(\mathbb{B}_{c}\rightarrow\star\right)\right)\rightarrow C\,true_{c}\rightarrow C\,true_{c}}\left(C:\left(\mathbb{B}_{c}\rightarrow\star\right)\right)\rightarrow C\,true_{c}\rightarrow C\,false_{c}\right)\,toLogic\,tt_{c}$\tabularnewline
let $toLogic'\coloneqq toLogic::\left(\mathbb{B}_{c}\rightarrow\star\right)$ & \tabularnewline
$\rightsquigarrow$ & $\left(\left(refl_{true_{c}:\mathbb{B}_{c}}\,toLogic'::toLogic'\,true_{c}\rightarrow toLogic'\,true_{c}\right)::toLogic\,true_{c}\rightarrow toLogic\,false_{c}\right)\,tt_{c}$\tabularnewline
$refl_{true_{c}:\mathbb{B}_{c}}\coloneqq\lambda-\,cx\Rightarrow cx$ & $\left(\left(\left(\lambda-\,cx\Rightarrow cx\right)\,toLogic'::toLogic'\,true_{c}\rightarrow toLogic'\,true_{c}\right)::toLogic\,true_{c}\rightarrow toLogic\,false_{c}\right)\,tt_{c}$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(\lambda cx\Rightarrow cx\right)::toLogic'\,true_{c}\rightarrow toLogic'\,true_{c}\right)::toLogic\,true_{c}\rightarrow toLogic\,false_{c}\right)\,tt_{c}$\tabularnewline
let $tt_{c}'\coloneqq tt_{c}::toLogic\,true_{c}$ & \tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(\lambda cx\Rightarrow cx\right)tt_{c}'::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(tt_{c}'::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
 & $\left(\left(\left(tt_{c}::toLogic'\,true_{c}\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
 & $\left(\left(\left(tt_{c}::\left(toLogic::\left(\mathbb{B}_{c}\rightarrow\star\right)\right)\,true_{c}\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(toLogic\left(true_{c}::\mathbb{B}_{c}\right)::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$toLogic\coloneqq\lambda b\Rightarrow b\,\star\,Unit_{c}\,\perp_{c}$ & $\left(\left(\left(tt_{c}::\left(\left(\lambda b\Rightarrow b\,\star\,Unit_{c}\,\perp_{c}\right)\left(true_{c}::\mathbb{B}_{c}\right)::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(\left(true_{c}::\mathbb{B}_{c}\right)\,\star\,Unit_{c}\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(\left(true_{c}::\mathbb{B}_{c}\right)\,\star\,Unit_{c}\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\mathbb{B}_{c}\coloneqq\left(X:\star\right)\rightarrow X\rightarrow X\rightarrow X$ & $\left(\left(\left(tt_{c}::\left(\left(true_{c}::\left(\left(X:\star\right)\rightarrow X\rightarrow X\rightarrow X\right)\right)\,\star\,Unit_{c}\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(\left(true_{c}\left(\star::\star\right)::\left(\star\rightarrow\star\rightarrow\star\right)\right)\,Unit_{c}\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\vdash true_{c}\coloneqq\lambda-\,x\,-\Rightarrow x$ & $\left(\left(\left(tt_{c}::\left(\left(\left(\lambda-\,x\,-\Rightarrow x\right)\left(\star::\star\right)::\left(\star\rightarrow\star\rightarrow\star\right)\right)\,Unit_{c}\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(\left(\left(\lambda x\,-\Rightarrow x\right)::\left(\star\rightarrow\star\rightarrow\star\right)\right)\,Unit_{c}\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(\left(\left(\lambda x\,-\Rightarrow x\right)\,\left(Unit_{c}::\star\right)::\left(\star\rightarrow\star\rightarrow\star\right)\right)\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(\left(\left(\lambda-\Rightarrow\left(Unit_{c}::\star\right)\right)::\left(\star\rightarrow\star\right)\right)\,\perp_{c}::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
...$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::\left(\left(Unit_{c}::\star\right)::\star\right)\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
...$\rightsquigarrow$ & $\left(\left(\left(tt_{c}::Unit_{c}\right)::toLogic'\,true_{c}\right)::toLogic\,false_{c}\right)$\tabularnewline
\end{tabular}

% TODO if it is possible to prove the equivelence, grey out or skip the redundant casts

$\left(\lambda pr\Rightarrow pr\,toLogic\,tt_{c}\right)\left(refl_{true_{c}:\mathbb{B}_{c}}::_{true_{c}\doteq_{\mathbb{B}_{c}}true_{c}}true_{c}\doteq_{\mathbb{B}_{c}}false_{c}\right)$
...
$\lambda b\Rightarrow b\,\star\,Unit_{c}\,\perp_{c}$

...

$\left(\lambda pr.pr\,\left(\lambda b:\mathbb{B}_{c}.b\,\star\,\star\,\perp\right)\,\perp\qquad:\lnot true_{c}=_{\mathbb{B}_{c}}false_{c}\right)\left(refl_{true_{c}:\mathbb{B}_{c}}::true_{c}=_{\mathbb{B}_{c}}true_{c}=_{l}true_{c}=_{\mathbb{B}_{c}}false_{c}\right)$
...
Recall that we proved $\lnot\star=_{\star}\perp$ what happens if
we none the less assume it? Every type equality assumption needs an
underlying term, here we can choose $refl_{\star:\star}:\star=_{\star}\star$,
and cast that term to $\star=_{\star}\perp$ resulting in $refl_{\star:\star}::_{\star=_{\star}\star}\star=_{\star}\perp$.
Recall that $\lnot\star=_{\star}\perp$ is a short hand for $\star=_{\star}\perp\rightarrow\perp$
...
\section{old proof}
\begin{itemize}
\item Every term well typed in the surface language elaborates 
\begin{enumerate}
\item if $\Gamma\vdash$, then there exists $H$ such that $\Gamma\,\textbf{Elab}\,H$ 
\item if $\Gamma\vdash$, $\Gamma\vdash m\overrightarrow{\,:\,}M$ then
there exists $H$, $a$ and $A$ such that $\Gamma\,\textbf{Elab}\,H$,
$H\vdash m\,\textbf{Elab}\ a\overrightarrow{\,:\,}A$ 
\item if $\Gamma\vdash$, $\Gamma\vdash m\overleftarrow{\,:\,}M$ and given
$\ell$, $o$then there exists $H$, $a$and $A$ such that $\Gamma\,\textbf{Elab}\,H$,
$H\vdash\textbf{Elab}\,a\,m\overleftarrow{\,:_{\ell o}\,}A$ 
\end{enumerate}
\end{itemize}
TODO can $\Gamma\textbf{Elab}H$ be made simpler?
...
$\operatorname{\overrightarrow{ty}-\star}$ &  & \tabularnewline
 & ... & mutual induction\tabularnewline
 & $H\vdash\star\,\textbf{Elab}\,\star\overrightarrow{\,:\,}\star$ & $\operatorname{\overrightarrow{\textbf{Elab}}-\star}$\tabularnewline
$\operatorname{\overrightarrow{ty}-::}$ & $\Gamma\vdash m\overleftarrow{\,:\,}M$, for some $\ell$ & ( $\ell$ comes from the syntactic extension to the surface lang)\tabularnewline
 & $H\vdash m\overleftarrow{\,:_{\ell,.}\,}A\,\textbf{Elab}\,a$ & by induction, $o=.$\tabularnewline
 & $\Gamma\vdash M\overleftarrow{\,:\,}\star$ & by regularity\tabularnewline
 & $H\vdash M\overleftarrow{\,:_{\ell.}\,}\star\,\textbf{Elab}\ A'$ & by induction\footnote{not well founded?}\tabularnewline
 & m\textasciitilde a, M\textasciitilde A, m:M then a:A & TODO\tabularnewline
 & $H\vdash\left(m::_{\ell}M\right)\textbf{Elab}\,a\overrightarrow{\,:\,}A$ & $\operatorname{\overrightarrow{\textbf{Elab}}-::}$\tabularnewline
$\operatorname{\overrightarrow{ty}-fun-ty}$ & \multicolumn{2}{l}{$\Gamma\vdash M\overleftarrow{\,:\,}\star\quad\Gamma,x:M\vdash N\overleftarrow{\,:\,}\star$,
with $\ell$, $\ell'$}\tabularnewline
...
\end{tabular}
\begin{enumerate}
\item Blame never points to something that checked in the bidirectional
system 
\begin{enumerate}
\item if $\vdash m\overrightarrow{\,:\,}M$, and $\vdash\textbf{Elab}\ m\,a\overrightarrow{\,:\,}A$
, then for no $a\rightsquigarrow^{*}a'$ will $\textbf{Blame}\:\ensuremath{\ell}\,o\:a'$
occur 
\end{enumerate}
\end{enumerate}
