...
Type assignment systems are convenient to study the theory of a dependently typed language, because terms do not need to contain information to help type checking, complicating the syntax. %type annotations. %and will be easier to work with than other styles of typing that require variables to be annotated.
% For instance, $\mathsf{fun}\,f\,x\Rightarrow m$ does not require a type be given for $f$ or $a$.
\todo{church/curry?}\todo{awk}
...
% \end{proof}
\begin{lem}
$\Rrightarrow$ is closed under substitutions that step.\todo{step vs. par reduce}
...
$\forall{\color{red}m},{\color{red}m'},{\color{red}m''}.\:{\color{red}m\Rrightarrow m'}\:\wedge\:{\color{red}m\Rrightarrow m''}\: \mathrm{implies}\: {\color{red}m'}{\color{blue}\Rrightarrow max\left(m\right)} \:\wedge\: {\color{red}m''}{\color{blue}\Rrightarrow max\left(m\right)}$
...
\item In the case of \rulename{\Rrightarrow-\mathsf{fun}-app}, $m'\Rrightarrow \textbf{max}\left(m\right)$ by \rulename{\Rrightarrow-\mathsf{fun}-app-red}.
\end{casenv}
\end{proof}
\begin{lem}
...
However this induction does not go through directly, and the induction hypothesis must be extended to definitional equality. \todo{awk}
...
\begin{casenv}
 ...
 \item All other cases are impossible.\todo{end impossible cases with !}
\end{casenv}
...
The second key theorem to show is called progress.\todo{awk, it's name is the least interasting thing}
...
We can choose a \cbv{} relation since% this meets all the properties required, and 
