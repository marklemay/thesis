...
When is an expression reasonable?
The expression $\star\star\star\star$ is allowed by the syntax of the language, but seems dubious.
%Since $\star$  is not a function,  it has no way of being given an input by application.
Type systems can disallow $\textbf{Stuck}$ terms like these from ever occurring, which in turn prevents bad runtime behavior.
...
\todo{church/curry?}
Practically this means that the type assignment system should\ccnote{keeping as should since you CAN use the rules as an inefficient procedure... but you shouldn't} not be used as a type checking algorithm since it may need to ``infer'' an unrealistic amount of information.
This also means that terms do not necessarily have unique typings.\ccnote{don't think the MGU is salient here}
...
The preservation theorem shows that typing information is invariant over evaluation.\ccnote{I'm using evaluation since this can apply to both computation relations}
...
Where $\sigma$, $\tau$ are substitutions.
...
} when, for all $m$, $n$, $n'$, if $mRn$ and $\:mRn'$ then there exists $n''$ such that $nRn''$ and $n'Rn''$.
...
\item In the case of \rulename{\Rrightarrow-\mathsf{fun}-app}, $m'\Rrightarrow \textbf{max}\left(m\right)$ by \rulename{\Rrightarrow-\mathsf{fun}-app-red}.\ccnote{All other cases follow directly by induction (the confusion may have come from the way I originally noted the max function)}
\end{casenv}
\end{proof}
...
\begin{proof}
By induction.
Intuitively by repeated application of the diamond property, ``tiling'' the interior region.
...
\]\ccnote{I am pervasively assuming variables behave correctly over binding constructs, so I should probably note that up front}
\end{thm}
\begin{proof}
...
The progress theorem states: for a well typed term in an empty context, then a further \cbv\footnote{
  It is tempting to use $\Rrightarrow$ as the main notion of reduction, since it corresponds to $\equiv$.
  However, since $\Rrightarrow$ is reflexive, no expression could get stuck.
} step can be taken or computation is finished.
...
If $\tasys v\,:\,P$ and $P\equiv\left(x:N\right)\rightarrow M$ then $v=\mathsf{fun}\,f\,x\Rightarrow m$, for some $m$. \ccnote{don't want to quantify over the bound variables since that implies a non de bruijn interpretation}
\end{lem}
\begin{proof}
...
\item \rulename{ty-conv} by the equivalence\ccnote{keeping this "equivalence" instead of "transitivity" because I think you also need to use symmetry} of $\equiv$ and induction
...
The \slang{} has type soundness:

\begin{thm} Type Soundness.
 
If $\tasys m\,:\,M$ and $m\rightsquigarrow_\ast m'$ then $m'$ cannot be $\textbf{Stuck}$.
\end{thm}
\begin{proof}
