...
While $Unit_{c}$ can be interpreted as a trivially true proposition, since it has only one good inhabitant\footnote{
  ...
  $\mathbb{B}$ is a collection of 2 tags $true$ and $false$, above we could have renamed the tags.
  Nothing but convention informs the meaning.
  If-then-else constructs could be turned into if-else-then without changing anything essential.

  In type theory the false proposition contains no proofs, if you ever get one, something has gone wrong and you have an excuse to do anything.
...
Since the definition of $Even_{c}\,2_{c}$ expands to $(Unit_{c} \rightarrow \perp_{c}) \rightarrow \perp_{c}$, since we have a function $s : (Unit_{c} \rightarrow \perp_{c})$ we only need to give it a member of $Unit_{c}$ to satisfy the type constraint.
...
By the expansion of definitions $tt_{c}\ :\ Even_{c}\,0_{c}$.
