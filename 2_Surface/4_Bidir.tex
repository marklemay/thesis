...
This has the advantage of precisely limiting where the $\rulename{ty-conv}$ rule can be uesed, since conversion checking is usually an inefficient part of dependent type checking.
...
Unlike the undirected judgments of the Type Assignment System, the inference rule of the \bidir{} system does not associate defineitionally equavelent types.
...
The similar issue is exixtes with check rules around function definitions.
...
With that caveate, this document opts for unevauated version of the rules to make some properties easier to present and prove.
...
But it would be difficult for the system described here since type annotations evaluate away, contrediction the normal preservation lemma.
