...
When an inference meets a check, a conversion verifies that the types are definitionally equal.\ccnote{keeping this as is since this sentence is talking about bidirectional systems in general, and it is acurate}
...
To save space we can instead show that for every well typed \ac{TAS} expression there is an equivelent \bidir{} expression, though annotations may need to be added (or removed). % no need to remove if properly \bidir{}).
