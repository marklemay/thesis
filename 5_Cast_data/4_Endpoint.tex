...
We can recover some of the familar judgments in \Fref{cast-data-endpoint-rules-def}.
A term has well-cast when it is its own endpoint (formalized in the \rulename{ty-def} rule).
Therefore a well-cast term can only have assert and unify syntax in proprer positions of casts. 
Also we can suggestively write endpoints as an equality with $\approx$.
...
The function is defined in \Fref{cast-Data-index-1}.
