...
Where $x$ has the type $\Vect{}\,A\,(\mathtt{S}\,n)$.

What can go wrong in the presence of casts?
\begin{itemize}
\item
A blamable cast may have made $x$ appear to be a Vector even when it is not.
...
Unlike in \ch{3}, casts will need to be generalized to contain evidence of equality.
 
In the the second branch, the pattern match gives $A:\star$, $p:\mathtt{Vec}\ A\ \mathtt{Z}\approx\mathtt{Vec}\,\mathtt{Bool}\,(\mathtt{S}\,y)$.
The body of the branch encodes the contradiction using explicit blame syntax by observing $\mathtt{Z}\neq\mathtt{S}\,y$ with $TCon_{1}p$.
...
\item
If the \Vect{} is inhabited by an incorrect type, such as $\mathtt{Cons}\,\mathtt{Nat}\,3\,...\,...::\mathtt{Vec}\,\mathtt{Bool}\,5$, the case will return $3::\mathtt{Bool}$ with a cast that rests on the blamable assertion of $\mathtt{Vec}\,\mathtt{Nat}\,5\approx\mathtt{Vec}\,\mathtt{Bool}\,\,5$.
...
Using a \whnf{} strategy, as in the implemented elaborator, the blame will be embedded in the resulting term and discovered whenever that term is eliminated.
\end{itemize}
...
\begin{itemize}
\item
This means the elaborator can construct $TCon_0(p2):Nat'\approx\mathtt{Nat}$, $TCon_0(p1):Nat''\approx Nat'$ and can combine the evidence to show $TCon_0(p1) \cup TCon_0(p2):Nat''\approx\mathtt{Nat}$.
\item
The elaborator knows from unification what the type of every sub expression is supposed to be, so casts can be injected using evidence from the pattern.
\end{itemize}
\item
...
We can now address possible but unstated branches that were alluded to in \Ch{4}.
...
Where $N:\star$, $a:N$, and $p:\mathtt{Id}\ N\ a\ a\approx\mathtt{Id}\ \mathtt{Nat}\ 2\ 2$.
...
If we have evidence from $q\ :\ (a ::(TCon_0(p)))\ \approx\ 2$ then $\Vect{}\, \Bool{}\, \underline{q}\ :\ \Vect{}\, \Bool{}\, \underline{(a :: (TCon_0(p)))}\ \approx\ Vect{}\, \Bool{}\, \underline{2}$.
...
\end{itemize}
In general there is no way around this, equality evidence may be constructable in subtle ways, and we can't check everything.
...
$\mathtt{peek}$ will ignore several discrepancies in the index of the \Id{} type, if run in \whnf{}\footnote{
  ...
  $\mathtt{peek' : Id (Unit -> Nat) (\\ - => 0) (\\ - => 1) -> Unit -> Nat}$.
}.
