...
The standard workflow seems sufficient for type systems in many mainstream typed programming languages. \todo{Ramsy flagged something around here but can't see the message}
...
If there are no warnings, the programmer could call their program a proof along the lines of the Curry-Howard correspondence\footnote{
  In the system presented here, the programmer will need to argue all relevant constructions are well founded to claim that they have a proof.
}\todo{cite?}.
...
While apparently a simple idea, the technical details required to manage checks that delay until runtime in a dependently typed language are fairly involved.\todo{awk}
...
Then an Elaboration procedure takes most (untyped) terms of the surface syntax into terms in the \clang{} is presented.
...
Surprisingly the inclusion of dependent data requires several changes to the system in \Ch{3}, since finer observations are observable.
