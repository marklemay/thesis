...
For instance, recursively generating a vector of a given length as in the $\mathtt{rep}$ funciotn in \Fref{data-elim-examples}.
...
In general, the motive will be treated like the typing annotations in \ch{2}, in that the \ac{TAS} will only allow correct motives in a well typed term, motives will not be required and the motive will be definitionally irrelevant.
...
The grammar includes a little more syntax than is strictly necessary, since the $\Bool{},1$ list could be inferred and the $y\Rightarrow z\Rightarrow$ binders are not needed in the branch.
