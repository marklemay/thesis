...
data PrePath {d : D} : Set where
  ...
  Assert : (a a' : PreSyntax {d})
    -> Loc -> Obs {d} -- if observations live here, it is hard to seperate out function inputs.
    -> PreSyntax {bind 1 d}
    -> PrePath
--  Refl : PrePath
...
data PreSyntax {d} where
  ...
  Cast : (bod : PreSyntax {d})
    ...
    -> ChoiceTree {PrePath {unseperate d}} {D.sepV d}
    -- -> (ty : PreSyntax {d})
    -> PreSyntax
  ...
  Same : (i : Fin (D.sepV d)) -> PreSyntax {fixChoice d i} -> PreSyntax {d} -> PreSyntax
